...
parser = argparse.ArgumentParser(description='Fix "Activity" boxes in html files prepared by OOsplit.py so they use PB textboxe.')
...
parser.add_argument("-l", "--logfile", help='Filename for logfile to which will be appended detailed progress information; default is "fix_activities.log".', default="fix_activities.log", type=argparse.FileType('a'))
...
parser.add_argument('-u', '--updating_manifest', help='filename of new manifest which can be used with OOreup.py to correct the activity boxes in the PB book; dafault is "manifest.update_activities"', default='manifest.update_activities')
  ...
  if activities_this_file:
    ...
    if args.backup_changed_files:
      ...
      for l in old_contents:
        old_fh.write(l)
      old_fh.close()
    activities_fixed += activities_this_file
if activities_fixed:
  log_and_print(f'Fixed {activities_fixed} Activities in {len(files2fix)} files')
  update_fh.close()
else:
  log_and_print("No Activities needed fixing!")
